...
std::string decode_dynamic_byte_remapping(const std::string& sdbr,
    ...
    // Iterate over SDBR with a step of the length of each mapped value
    for (size_t i = 0; i < sdbr.size(); i += mappings.begin()->second.size()) {
        // Extract the substring corresponding to a mapped value
        std::string substring = sdbr.substr(i, mappings.begin()->second.size());
...
int main() {
    ...
    std::ifstream sdbr_file("dbr_encoded_sdbr.txt");
